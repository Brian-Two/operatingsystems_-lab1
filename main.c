...
int main() {
  ...
  list_print(mylist);
  if(strcmp("90->70->60->50->40->20->10->5->NULL",listToString(mylist)) != 0)
  {
        printf("list_add_to_back : FAILED\n");
  }

  list_add_at_index(mylist, 25, 6); // insert before index 6 (1-based)
  list_print(mylist);
  if(strcmp("90->70->60->50->40->25->20->10->5->NULL",listToString(mylist)) != 0)
  {
        printf("list_add_at_index : FAILED\n");
  }

  if(!list_is_in(mylist, 25) || list_is_in(mylist, 999))
  {
        printf("list_is_in : FAILED\n");
  }

  x = list_remove_from_back(mylist);
  if (x != 5) { printf("list_remove_from_back : FAILED (got %d)\n", x); }
  list_print(mylist);

  x = list_remove_from_front(mylist);
  if (x != 90) { printf("list_remove_from_front : FAILED (got %d)\n", x); }
  list_print(mylist);
  return 0;
}